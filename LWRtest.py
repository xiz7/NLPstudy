...
if __name__ == "__main__":
    srcX, y = LWR.loadDataSet('data.txt');

    m,n = srcX.shape
    srcX = np.concatenate((srcX[:, 0], np.power(srcX[:, 0],2)), axis=1)
    # 特征缩放
    X = LWR.standardize(srcX.copy())
    ...
    predicateX = LWR.standardize(np.matrix([[8, 64]]))

    predicateX = np.concatenate((np.ones((1,1)), predicateX), axis=1)

    result, t = LWR.lwr(rate, maxLoop, epsilon, X, y, predicateX, 1)
    theta, errors, thetas = result

    result2, t = LWR.lwr(rate, maxLoop, epsilon, X, y, predicateX, 0.1)
    ...
    for i in range(50):
        ...
        yHat1.append(LWR.h(theta, x.T))
        yHat2.append(LWR.h(theta2, x.T))
